...
requiredPackages = c('tidyverse', 'broom', 'knitr', 'Hmisc', 'corrplot', 'emmeans', 'ggsignif', 'PresenceAbsence', 'languageR', 'psycho', 'ordinal', 'DHARMa', 'sjPlot', 'htmltools', 'pwr', 'effectsize', 'datawizard', 'parameters', 'car')
...
use odds ratio as an exponentiation of the logodds (logit) coefficients.
...
#### Cohen's *d* interpretation rules and t.test

Based on J. Cohen (1988) rule of thumb, we can interpret Cohen's *d*
...
```{r}
english %>% 
  t.test(RTlexdec ~ AgeSubject, data = ., var.equal = FALSE)
```
...
pwr.t.test(n = 2284, d = 2, sig.level = 2.2e-16)
```


The results here show that with a sample size of 2284, a large effect size of 2 and a significance level of 2.2e-16, we have a power of 1 (or 100\%). This is a massive power and effect size, but we know that the results are inflated de to not using a mixed effects model.
...
```{r}
cohen.ES(test = "t", size = "large")
pwr.t.test(n = 30, d = 0.8, sig.level = 0.01)
```


The power is only at 66.78%! Not very powerful. How many students should we observe for a test with 80% power?


```{r}
pwr.t.test(d = 0.8, power = 0.80, sig.level = 0.01)
```


We need at least 38 students per group. If we lower our p value, the sample size will increase drastically
...
```{r}
p.out <- pwr.t.test(d = 0.2, power = 0.80, sig.level = 0.05)
p.out
plot(p.out)

p.out <- pwr.t.test(d = 0.5, power = 0.80, sig.level = 0.05)
p.out
plot(p.out)

p.out <- pwr.t.test(d = 0.8, power = 0.80, sig.level = 0.05)
p.out
plot(p.out)

p.out <- pwr.t.test(d = 2, power = 0.80, sig.level = 0.05)
p.out
plot(p.out)
```
...
```{r}
p.out <- pwr.t.test(d = 0.2, power = 0.80, sig.level = 0.01)
p.out
plot(p.out)

p.out <- pwr.t.test(d = 0.5, power = 0.80, sig.level = 0.01)
p.out
plot(p.out)

p.out <- pwr.t.test(d = 0.8, power = 0.80, sig.level = 0.01)
p.out
plot(p.out)

p.out <- pwr.t.test(d = 2, power = 0.80, sig.level = 0.01)
p.out
plot(p.out)
```
...
We are now interested in looking at group differences. But before moving
into linear models, let's assume we simply wanted to use a student's
t.test to evaluate the differences between two groups. In our case, we
have two age groups within the variable `AgeSubject` (young vs old).
Before running a t.test, there are some assumptions one needs to take
into account:
...
To check normality of distribution, we use the `shapiro.test` on the
numeric outcome. Given that our predictor now has two levels.

```{r}
shapiro.test(english$RTlexdec)
```

How to interpret this statistically significant result? This tells us
that the distribution of the data is statistically different from a
normal distribution.

#### Density plot

We can also use a density plot to evaluate normality. The results show
that both levels have bell shaped distributions but they are both skewed
to the right (positively skewed data). See [this
website](https://www.cambridgemaths.org/blogs/skewed-usage-skewed-distribution/)
for a nice example of left and right skwed data distributions.
...
If our data were normally distributed, we can use the `bartlett` test.
If our data were non-normally distributed, we would use the Levene Test
(either with `var.test` from base-R or `leveneTest` from the car package
We can check both.
...
```{r}
english %>% 
  bartlett.test(RTlexdec ~ AgeSubject, data = .)
```
...
```{r}
english %>% 
  var.test(RTlexdec ~ AgeSubject, data = .)
```
...
```{r}
english %>% 
  car::leveneTest(RTlexdec ~ AgeSubject, data = .)
```

In all cases, the statistically significant result indicates that there
is evidence that the variance of two levels of the factor `AgeSubject`
is statistically significant; i.e., the variances are not equal. This is
important as we will use this in our t-test later on.

### First steps

Up to now, we have looked at descriptive statistics, and evaluated
summaries, correlations in the data (with p values), and checked the
normality of distribution of our data.
...
We then run a t-test on the data. We specify the formula as `y ~ x` and
add `var.equal = FALSE` (based on the normality tests above)
...
To interpret the t-test, we say that there is evidence for a
statistically significant difference between the two groups: `young`
and `old`: `t(4534.6) = 67.468, p < 2.2e-16`. The mean of
`RTLexdec` for the young group is significantly lower than that of the
old group.
...
The results of the linear model are exactly the same, albeit with a
difference in the sign of the difference. This indicates that the
$\beta$ coefficient for `young` is significantly lower than that
of `old` by `-0.221721`: `t = -67.47, p < <2e-16`.

The dataset `english` contains two levels for `AgeSubject.` We will run
an ANOVA and a linear model on the data.

#### Basic ANOVA

We can use the function `aov` to run an Analysis of Variance on the full
dataset
...
But wait... How is the linear model comparable to the analysis of
variance we ran above? This linear model derives the analysis of
variance we saw above, use `anova` on your linear model..
...
They are exactly the same... The underlying of an Analysis of variance
is a linear model. We will continue with a linear model to understand it
better
...
#### GLM as a classification tool {#GLM-Classification}
